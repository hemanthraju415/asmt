...
public class ProductsDALImpl implements IProductsDAL {
	...
	public Boolean searchService(Integer id, Integer pincode) {
		...
		try {
			psmt = con.prepareStatement(
					"select prod_id from i213_productsdata where prod_prct_id in(select prct_id  from i213_ProductCategoryWiseServiceableRegions where srrg_id in(select srrg_id from i213_ServiceableRegions where srrg_pinfrom<=? and srrg_pinto>=?))");
